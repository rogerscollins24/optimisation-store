...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    const refreshUnread = async () => {
      try {
        const unread = await adminGetSupportUnreadCount(token);
        setSupportUnreadCount(Math.max(0, unread));
      } catch {
        // Keep existing value if endpoint fails.
      }
    };

    void refreshUnread();
    const interval = window.setInterval(() => void refreshUnread(), 10000);

    const onUnreadUpdated = (event: Event) => {
      const customEvent = event as CustomEvent<{ count?: number }>;
      if (typeof customEvent.detail?.count === 'number') {
        setSupportUnreadCount(Math.max(0, customEvent.detail.count));
      }
    };

    window.addEventListener('support-unread-updated', onUnreadUpdated as EventListener);
  ...
  useEffect(() => {
    ...
    void adminMarkAllSupportMessagesRead(token)
      .then(() => {
        setSupportUnreadCount(0);
      })
      .catch(() => undefined);
            ...
            return (
              ...
              >
                ...
                {item.href === '/support' && supportUnreadCount > 0 ? (
                  <span className="min-w-5 h-5 px-1.5 rounded-full bg-rose-500 text-white text-[11px] font-semibold inline-flex items-center justify-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="p-2 rounded-lg text-slate-400 hover:bg-slate-800/50 hover:text-slate-200 transition-colors relative">
              ...
              {supportUnreadCount > 0 ? (
                <span className="absolute -top-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-rose-500 text-white text-[10px] font-semibold inline-flex items-center justify-center">
                  {supportUnreadCount > 99 ? '99+' : supportUnreadCount}
                </span>
              ) : null}
            </button>
