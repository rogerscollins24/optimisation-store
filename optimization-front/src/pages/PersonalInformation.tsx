...
export default function PersonalInformation() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json().catch(() => ({ detail: 'Unable to save personal information' }));
        throw new Error(error.detail || 'Unable to save personal information');
      }

      await refreshUser();
      navigate('/profile');
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Unable to save personal information');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="mx-auto max-w-2xl rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
          <div className="grid gap-4">
            ...
            <div>
              ...
              <input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Enter your email" className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-800 outline-none focus:border-blue-500" />
            </div>

            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700">Phone</label>
              <input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Enter your phone number" className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-800 outline-none focus:border-blue-500" />
            </div>
