...
import { formatLocalizedTime } from '../lib/dateFormatting';
...
export default function ChatModal({ token, presetMessage, presetSubject, openSignal }: ChatModalProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-110">
      {isOpen ? (
        <div className="w-90 h-140 rounded-2xl overflow-hidden shadow-2xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 flex flex-col">
          <div className="bg-linear-to-r from-cyan-500 to-teal-500 text-white px-5 py-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-cyan-100 mt-1 text-sm">{ticket?.status ? getStatusLabel(ticket.status) : t('newChat')}</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="flex-1 p-4 flex flex-col gap-4">
              <p className="font-semibold text-slate-700 dark:text-slate-200">{t('startSupportRequest')}</p>
              <input
                value={subject}
                onChange={(event) => setSubject(event.target.value)}
                placeholder={t('subject')}
                className="rounded-lg border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 dark:text-gray-100 dark:placeholder:text-gray-500 px-3 py-2"
              />
              <textarea
                value={draft}
                onChange={(event) => setDraft(event.target.value)}
                placeholder={t('enterYourMessage')}
                className="rounded-lg border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 dark:text-gray-100 dark:placeholder:text-gray-500 px-3 py-2 min-h-30"
              />
            ...
            <>
              <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-zinc-800 p-4 space-y-3">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.is_admin_reply ? 'justify-start' : 'justify-end'}`}>
                    <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm ${msg.is_admin_reply ? 'bg-white dark:bg-zinc-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-zinc-600' : 'bg-cyan-500 text-white'}`}>
                      <p>{translateText(msg.content)}</p>
                      <p className={`text-[11px] mt-1 ${msg.is_admin_reply ? 'text-slate-400' : 'text-cyan-100'}`}>
                        {formatLocalizedTime(msg.created_at, locale)}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="border-t border-slate-200 dark:border-zinc-700 p-3 bg-white dark:bg-zinc-900">
                <div className="text-xs text-slate-400 dark:text-slate-500 mb-2">{isConnected ? t('connected') : t('reconnecting')}</div>
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder={t('enterYourMessage')}
                    className="flex-1 rounded-lg border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 dark:text-gray-100 dark:placeholder:text-gray-500 px-3 py-2 text-sm"
                  />
